...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	q := Queries{db: db}
	var err error
	if q.addBooksStmt, err = db.PrepareContext(ctx, addBooks); err != nil {
		return nil, fmt.Errorf("error preparing query AddBooks: %w", err)
	}
...
func (q *Queries) Close() error {
	var err error
	if q.addBooksStmt != nil {
		if cerr := q.addBooksStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing addBooksStmt: %w", cerr)
		}
	}
